...
| `subagent`              | `state: "started" \| "finished"`, `agentType?`, `agentId?`, `summary?`                     | Claude Code's `SubagentStart`/`SubagentStop` — tracked as a running count, not a boolean. `agentId` (Phase 5, Track A) correlates a started/finished pair; `summary` is `SubagentStop`'s `last_assistant_message`. |
