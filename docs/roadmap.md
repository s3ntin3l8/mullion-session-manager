...
- Does **not** depend on Phase 2's review gate (2.7) or hook socket (2.1) — task-prompt injection here is a plain environment variable at spawn time, same mechanism the roadmap already uses elsewhere before hooks exist. This is what makes pulling it forward possible: it only needs Phase 1/2 to be _stable_, not for 2.7 specifically to have shipped. (Phase 6's 6.2 later replaced this with `stashSeed`, a single-use seed consumed by the agent's own `SessionStart` hook — not a plain env var; see [`docs/tasks.md`](tasks.md). The thin slice's own mechanism, described here, predates that and was never itself an env var either by the time 2.5.2 actually shipped — this note is retained as a description of the _original_ design intent, not current behavior.)
...
#477), 6.7 (#220, #475), 6.8 (#283, #476), 6.9 (#233, #471). See
[`docs/tasks.md`](tasks.md) for the full design (lifecycle, agent
selection, safety envelope, GitHub sync, worktree lifecycle, and the
known-gaps list — per-task token scope and the missing kill-switch UI
toggle carry over unresolved from this phase).
