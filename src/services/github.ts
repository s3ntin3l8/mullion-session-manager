...
interface ExternalReviewWorkflowCacheEntry {
  value: boolean;
  expiresAt: number;
}
const externalReviewWorkflowCache = new Map<string, ExternalReviewWorkflowCacheEntry>();
// Workflow files change about never — same reasoning and same TTL as
// fetchRequiredStatusContexts's own cache above, and for the identical
// reason: read on every processAutoApprovals tick for every task that has
// entered the external-review stage, otherwise.
const EXTERNAL_REVIEW_WORKFLOW_CACHE_TTL_MS = 60 * 60_000;

/**
 * Sequential review phase (branchdam-mobile #83's investigation) — whether
 * a workflow file's `pull_request:` trigger explicitly opts into
 * `ready_for_review`, GitHub's own default `pull_request` types being
 * `opened`/`synchronize`/`reopened` (never `ready_for_review`) unless a
 * workflow lists it, which hermes-review.yml (and any equivalent
 * claude.yml/Codex review workflow) does.
 *
 * mullion-reviewer finding: an earlier version of this matched
 * `/pull_request/` and `/ready_for_review/` ANYWHERE in the file,
 * independently — a workflow that merely mentions `ready_for_review` in a
 * comment, a step name, or an `if:` guard unrelated to its own trigger
 * would false-positive, and a false positive here is NOT harmless: it
 * sends `resolveExternalReviewGate` into polling a review decision with NO
 * timeout (deliberate — see that function's own comment), so a false match
 * parks a task in `reviewing` forever, waiting on a verdict nothing will
 * ever produce. Scoped instead to lines actually inside the
 * `pull_request:` trigger's own indented sub-block, and specifically its
 * `types:` list (either GitHub Actions' inline-array or block-list YAML
 * form) — still no YAML parser, but no longer "anywhere in the file."
 */
function pullRequestTriggerAllowsReadyForReview(yamlText: string): boolean {
  const lines = yamlText.split("\n");
  for (let i = 0; i < lines.length; i++) {
    const triggerLine = /^(\s*)pull_request:\s*(#.*)?$/.exec(lines[i]);
...
/**
 * This is what lets task-reconciler.ts's external-review gate answer "does
 * a reviewer exist" immediately, with no detection window at all: a repo
 * with no such workflow resolves `false` on the very first check, and a
 * repo that has one resolves `true` just as fast — the only waiting left
 * is for that KNOWN-to-exist workflow to actually post its verdict, never
 * for "is there even a reviewer."
 *
 * Best-effort and fails to `false` on any lookup error (no workflows
 * directory, a 403, a network failure) — same posture as
 * fetchRequiredStatusContexts: this only degrades "wait for an external
 * review" to "proceed as if none is configured," never blocks a task
 * forever on a GitHub read failure. Cached per `owner/repo` for
 * `EXTERNAL_REVIEW_WORKFLOW_CACHE_TTL_MS`; only a successful lookup is
 * cached (mirroring fetchRequiredStatusContexts again) — a failure is
 * retried on the next call rather than latched to `false`.
 */
export async function repoHasExternalReviewWorkflow(
  token: string,
  owner: string,
  repo: string,
): Promise<boolean> {
  validateGitHubRepoRef(owner, repo);
  const key = `${owner}/${repo}`;
  const cached = externalReviewWorkflowCache.get(key);
  ...
  try {
    ...
    // A 404 (no workflows directory at all) is the common "no reviewer"
    // case, not an error — collapses to `false` the same as every other
    // non-ok response here.
    if (!listRes.ok) return false;
    const entries = (await listRes.json()) as { path: string; type: string }[];
    const workflowFiles = entries.filter((e) => e.type === "file" && /\.ya?ml$/.test(e.path));
    ...
    for (const file of workflowFiles) {
      const encodedPath = file.path
        ...
        .join("/");
      const rawRes = await githubApiFetch(
        `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${encodedPath}`,
        { headers: { ...headers, Accept: "application/vnd.github.raw" } },
      );
      if (!rawRes.ok) continue;
      const text = await rawRes.text();
      if (pullRequestTriggerAllowsReadyForReview(text)) {
        found = true;
        break;
      }
    }
