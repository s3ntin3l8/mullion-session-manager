...
| `MULLION_SSH_AUTH_SOCK` | _(empty)_            | path to a unix socket implementing the SSH agent protocol (typically a manual `ssh -R` tunnel), injected as `SSH_AUTH_SOCK` into every session; empty falls through to an inherited ambient `SSH_AUTH_SOCK`, then to the SSH-agent bridge's own materialized socket (`<SESSIONS_DIR>/ssh-agent.sock`) — the bridge socket is materialized regardless of whether a laptop has ever paired, so an unpaired host still resolves to that path (the socket just refuses connections until one does) — see [`ssh-agent.md`](ssh-agent.md)'s Precedence section for the full three-tier order |
...
| `MULLION_GATEWAY_LOGOUT_URL` | _(empty)_ | optional gateway sign-out target; root-relative or absolute HTTP(S), and requires `MULLION_TRUST_GATEWAY=true`                                                                             |
...
| `MULLION_TASK_RATE_LIMIT_GRACE_MINUTES` | `5`            | wall-clock minutes the reconciler waits after a `rate_limit` stop_failure before failing a task, so a transient subscription-quota exhaustion doesn't kill a task that could resume once the quota refreshes; `0` = opt out (fail immediately); Settings-overridable                                                                                                                          |
...
| Variable                                    | Default | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MULLION_SCAFFOLD_GENERATE_SANDBOX_ENABLED` | `true`  | opt-out for `/setup/generate`'s mandatory bwrap sandboxing of a generation turn. When `false`, a turn always runs unwrapped, regardless of whether bwrap is usable — the escape hatch for an agent whose writable-path needs aren't covered by `agentSandboxWritablePaths` and would otherwise hard-fail. `agy` still refuses to run unsandboxed even with this set (it needs `--dangerously-skip-permissions` to read the repo at all, and bwrap is its only containment — issue #1130/#1133). Read independently by each host that spawns a generation turn, never sent over the wire — set per agent host, not just on the primary |
...
| `PREVIEW_AUTH_DASHBOARD_URL` | _(empty)_ | optional absolute http(s) URL (e.g. `https://mullion.example.com/`) the static 401 page links to when `PREVIEW_AUTH_REQUIRED` rejects a bookmarked/direct preview navigation (issue #1310); rejected at boot if set to anything else, since `PREVIEW_BASE_HOST` is often a sibling label of the dashboard's own host, not something this process could derive it from                                                                                                                          |
...
> **cmdline-tools version matters.** The AVD device-profile list (`avdmanager
list device`) and the available-system-images list (`sdkmanager --list`) are
> both whatever the installed cmdline-tools package bundles. Modern Pixel
> profiles (Pixel 8/9/10) need **cmdline-tools ≥ 23**; upgrade in place with
...
| `SSH_AUTH_SOCK`          | set to `MULLION_SSH_AUTH_SOCK`'s configured path when that's non-empty; otherwise this session's inherited ambient `SSH_AUTH_SOCK` (if any) is left untouched; otherwise, on a host that materializes an SSH-agent bridge socket, set to that path (`<SESSIONS_DIR>/ssh-agent.sock`) — resolved once at process boot, not per session; see [`ssh-agent.md`](ssh-agent.md)'s Precedence section |
