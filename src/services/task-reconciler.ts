...
async function spawnReviewAgentNow(
  ...
  try {
    ...
    await unlinkFindingsFileIfPresent(app, backend, task.id, task.reviewRounds);
  ...
  await Promise.all(
    [...byHost.entries()].map(async ([hostId, hostRows]) => {
      ...
      for (const row of hostRows) {
        ...
        try {
          findings = await backend.readTaskReviewFindings(task.id, task.reviewRounds);
        } catch (err) {
          app.log.warn(
            { err, taskId: task.id, round: task.reviewRounds },
