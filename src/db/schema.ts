...
export const tasks = sqliteTable(
  "tasks",
  {
    ...
    // Task 259314's investigation (issue #1038) — the moment the machine
    // genuinely stopped acting on this task's own initiative, as opposed to
    // autoReturnRounds merely reaching the resolved cap. The two are NOT the
    // same instant: autoReturnTask bumps autoReturnRounds to the cap value
    // at the START of the task's LAST permitted round (the worker is still
    // running, and the review that follows it still spawns unconditionally
    // — see processPendingReviewSpawns, which has no cap check by design,
    // since that final review is the only thing telling a human what's
    // still broken). Deriving "needs a human" from autoReturnRounds alone
    // made the board claim the machine had stopped while it was still
    // actively working the branch. This column is the ground truth
    // instead: null means either not capped, or capped but the machine
    // hasn't announced it yet (worker still finishing, or the confirming
    // review still in flight); non-null means nothing further happens on
    // its own — genuinely safe for a human to touch the branch.
    //
    // Hermes review (PR #1040) — set whenever a capped task's review round
    // is durably ingested (processReviewingTasks, gated on `capReached`
    // alone), NOT only when that round wanted another round
    // (verdict "changes-requested"): a capped task whose FINAL review comes
    // back clean or inconclusive is just as genuinely parked — nothing
    // auto-returns a non-"changes-requested" verdict either way — but a
    // narrower gate left that case unannounced forever. Only THAT site
    // (and the two external-event cap-notice sites, CI/PR-comment) can post
    // an actual "needs a human" GitHub comment alongside the write, since
    // only they know they're at the cap; the ingest write on a clean/
    // inconclusive final round sets this column with no comment to match —
    // the column tracks "stopped," not "a specific comment landed." Set
    // BEFORE any GitHub comment post at the three cap-notice sites in
    // task-reconciler.ts (not after — a crash between the two must leave
    // the banner correct, not stuck on "in flight" forever), and cleared at
