...
lifecycle, and the known-gaps list). GitHub App-scoped writes
([#489](https://github.com/s3ntin3l8/mullion-session-manager/issues/489))
shipped as an opt-in slice — repo-scoped installation tokens for Task
Master's own writes, falling back to the shared PAT when no App is
configured; a reversal of issue #60's "not a GitHub App" resolution below,
deliberately narrowed to Task Master's writes only. The rest of the phase's
leftovers, previously undocumented as GitHub issues, are now filed under
the Phase 6 milestone: [#483](https://github.com/s3ntin3l8/mullion-session-manager/issues/483)–[#491](https://github.com/s3ntin3l8/mullion-session-manager/issues/491).
...
- Polling only, matching the existing GitHub integration pattern. Webhook-driven ingest (`labeled`/`closed`) shipped as a slice (`#490`) sharing the poll loop's own upsert logic; the poll loop remains the fallback, and `unlabeled` handling plus full webhook-registration reconciliation stay open.
