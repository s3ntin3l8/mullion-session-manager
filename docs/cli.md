...
- `bundle remove` — removes Mullion's bundle content from this host
  ...
  `sessions.injectMullionBundle` off so it stays removed. No confirmation
  prompt at the CLI layer, same as `session kill`/`preview delete` — a
  confirming UI is the dashboard panel's job, not this command's.
