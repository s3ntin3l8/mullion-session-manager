...
| `review_gate`           | agent sends `state: "waiting"`, `prompt: string`; Mullion resolves it with `state: "waiting" \| "approved" \| "denied" \| "lapsed"` | A pending decision (issue #178's review gate — see below). `"lapsed"` (issue #840/#844) is server-generated only — no agent ever sends it — meaning nobody ever answered, so the agent fell through to its own native prompt; distinct from a human `"denied"`.                            |
...
**Persistence note (issue #844):** gate state (`SessionInfo.gateState`/
`gatePrompt`) is NOT in-memory only — it's in `StoredStateFields` and
survives a restart via the per-session state file (issue #323). What
doesn't survive is the live `pendingGates` socket/timer in `hooks.ts`, which
lives only in that process's memory. A restart while a gate is `"waiting"`
therefore surfaces a fourth state, `"lapsed"`: nobody ever answered, so the
agent fell through to its own native prompt (the same outcome as a live
timeout, dropped connection, or duplicate concurrent gate — `"lapsed"` covers
all of these, not just the restart case). Mullion resolves this at two
points: a gate still pending at a _graceful_ shutdown is resolved to
`"lapsed"` before its socket is destroyed (`hooksPlugin`'s `onClose`); a
`"waiting"` gate restored from a state file after a hard crash/`kill -9` is
resolved to `"lapsed"` on reattach (`Session.spawn()`'s `savedState`
handling). Either way, the UI never shows a live-looking Approve/Deny for a
