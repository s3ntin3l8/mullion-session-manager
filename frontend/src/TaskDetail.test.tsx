...
describe("TaskDetail delete action", () => {
  ...
  // GitHub-linked, that the closed issue/merged PR stay on GitHub).
  ...
  it("shows Delete for a done GitHub-linked task, with closed-issue/merged-PR confirm copy", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Delete task" }));
    expect(
      screen.getByText(/closed issue and merged PR stay on GitHub, and the branch is untouched/),
    ).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Confirm delete" }));
