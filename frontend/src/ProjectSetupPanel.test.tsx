...
describe("ProjectSetupPanel", () => {
  ...
  // reverted checkbox — THAT one PATCHed injectWorkflowConventions, which
  // also feeds text resolution and would have broken drift tracking; this
  // one PATCHes the separate suppressConventionsInjectionAfterScaffold
  // column instead, which touches neither.
  describe("post-apply conventions-injection offer (issue #1208)", () => {
    const originalState = useDashboardStore.getState();
    afterEach(() => {
      useDashboardStore.setState(originalState, true);
    });
    ...
    it("offers to skip per-session injection after apply, and PATCHes the new column (never injectWorkflowConventions) when checked", async () => {
      const updateProject = vi.fn().mockResolvedValue(makeProject({ id: 1 }));
      useDashboardStore.setState({
        updateProject,
        settings: {
          ...originalState.settings,
          sessions: {
            ...originalState.settings.sessions,
            workflowConventionsText: "Our real, hand-authored conventions.",
          },
        },
        projects: [makeProject({ id: 1 })],
      });

      const user = await applyOnce();
      const checkbox = screen.getByLabelText("Skip per-session injection");
    ...
    it("does not show the offer when no install-wide conventions text is configured", async () => {
      useDashboardStore.setState({
        settings: {
          ...originalState.settings,
          sessions: { ...originalState.settings.sessions, workflowConventionsText: "" },
      ...
      await applyOnce();
      expect(screen.queryByText("Skip per-session injection")).not.toBeInTheDocument();
    });

    it("does not show the offer when the project has opted out via injectWorkflowConventions", async () => {
      useDashboardStore.setState({
        settings: {
          ...originalState.settings,
          sessions: {
