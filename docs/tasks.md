...
| -------------------------------- | -------------------------------------------- | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Whether Task Master runs at all  | `settings.taskMaster.enabled`                | `MULLION_TASK_MASTER_ENABLED` (`false`)        | Gates every _new_ piece of autonomous work: the watcher's GitHub ingest + auto-claim, the claim/approve/reject endpoints (all three refuse with 403 while off), and spawning a new review-agent session when a task enters `reviewing`. Does **not** gate an already-in-flight task's own lifecycle sync — the reconciler keeps enforcing the time budget and progressing `claimed`/`in_progress` tasks, and syncing those transitions (including the resulting labels/comments) to GitHub, regardless, by design: this is a safety net, not new work — only the review-agent spawn on top of a `reviewing` transition counts as new work and is gated. The local task board (create/edit/drag/delete) works regardless too — see the Flag semantics decision above. |
