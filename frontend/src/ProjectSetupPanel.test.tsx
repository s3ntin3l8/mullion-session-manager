...
describe("ProjectSetupPanel", () => {
  ...
  // Issue #1205, Phase 3 — the post-apply offer to turn off per-session
  // injection now that the committed text and the injected text are the
  // same bytes. "Not an automatic write": nothing is PATCHed until the
  // user explicitly clicks "Done".
  describe("turn off per-session injection after apply (Phase 3)", () => {
    const originalState = useDashboardStore.getState();
    afterEach(() => {
      useDashboardStore.setState(originalState, true);
    });

    it("offers the checkbox, checked by default, and PATCHes injectWorkflowConventions:false on Done", async () => {
      const updateProject = vi.fn().mockResolvedValue({});
      useDashboardStore.setState({
        projects: [makeProject({ id: 1, injectWorkflowConventions: null })],
        updateProject,
      });
      vi.stubGlobal(
        "fetch",
    ...
    it("does not PATCH when the checkbox is unchecked before clicking Done", async () => {
      const updateProject = vi.fn().mockResolvedValue({});
      useDashboardStore.setState({
        projects: [makeProject({ id: 1, injectWorkflowConventions: null })],
        updateProject,
      ...
      await user.click(
        await screen.findByLabelText("Turn off per-session conventions injection for this project"),
      );
      await user.click(screen.getByText("Done"));
    ...
    it("clicking 'Scaffold another' never PATCHes, even with the checkbox left checked", async () => {
      const updateProject = vi.fn().mockResolvedValue({});
      useDashboardStore.setState({
        projects: [makeProject({ id: 1, injectWorkflowConventions: null })],
        updateProject,
      ...
      await user.click(screen.getByText("Scaffold another"));

      expect(updateProject).not.toHaveBeenCalled();
    });

    it("does not offer the checkbox at all when the project has already opted out", async () => {
      useDashboardStore.setState({
        projects: [makeProject({ id: 1, injectWorkflowConventions: false })],
      });
      vi.stubGlobal(
      ...
      expect(
        screen.queryByLabelText("Turn off per-session conventions injection for this project"),
      ).not.toBeInTheDocument();
    });
  });
});
